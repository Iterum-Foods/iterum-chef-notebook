#!/usr/bin/env python3
"""
Simple Ingredients Population Script
Works with existing SQLAlchemy database to populate ingredients table
"""
...
# Import only what we need for ingredients
...
# Create a minimal database setup just for ingredients
Base = declarative_base()

class SimpleIngredient(Base):
...
def populate_database():
    ...
    try:
        ...
        existing_ingredients = {ing.name.lower() for ing in db.query(SimpleIngredient).all()}
        ...
        print(f"📈 Total ingredients in database: {db.query(SimpleIngredient).count()}")
        
        # Show categories summary
        categories = db.query(SimpleIngredient.category).distinct().all()
        print(f"\n📂 Ingredient categories:")
        for category in categories:
            count = db.query(SimpleIngredient).filter(SimpleIngredient.category == category[0]).count()

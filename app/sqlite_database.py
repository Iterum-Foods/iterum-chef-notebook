...
class CulinaryDatabase:
    ...
    def save_ingredient_list(self, list_data: Dict[str, Any]) -> bool:
        """Save an ingredient list to database."""
        try:
            with sqlite3.connect(self.db_path) as conn:
                cursor = conn.cursor()
                cursor.execute('''
                    INSERT OR REPLACE INTO ingredient_lists 
                    (id, user_id, name, description, category, ingredients, is_favorite, created_at, updated_at)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
                ''', (
                    list_data['id'],
                    list_data['user_id'],
                    ...
                    list_data['category'],
                    list_data['ingredients'],
                    list_data['is_favorite'],
            ...
            print(f"Error saving ingredient list: {e}")
            return False
    
    def get_ingredient_lists(self, user_id: str, category: Optional[str] = None, search: Optional[str] = None) -> List[Dict[str, Any]]:
        """Get all ingredient lists for a user."""
        try:
            with sqlite3.connect(self.db_path) as conn:
                cursor = conn.cursor()
                
                query = '''
                    SELECT id, name, description, category, ingredients, is_favorite, created_at, updated_at
                ...
                for row in rows:
                    ...
                    # Parse ingredients count for summary
                    try:
                        ingredients = json.loads(row[4])
                        list_data['ingredient_count'] = len(ingredients)
                    except:
                        list_data['ingredient_count'] = 0
            ...
            print(f"Error getting ingredient lists: {e}")
            return []
    
    def get_ingredient_list(self, list_id: str, user_id: str) -> Optional[Dict[str, Any]]:
        """Get a specific ingredient list."""
        try:
            with sqlite3.connect(self.db_path) as conn:
                cursor = conn.cursor()
                cursor.execute('''
            ...
            print(f"Error getting ingredient list: {e}")
            return None
    
    def update_ingredient_list(self, list_data: Dict[str, Any]) -> bool:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                cursor.execute('''
                    ...
                    SET name = ?, description = ?, category = ?, ingredients = ?, 
            ...
            print(f"Error updating ingredient list: {e}")
            return False
    
    def delete_ingredient_list(self, list_id: str, user_id: str) -> bool:
        """Delete an ingredient list."""
        try:
            with sqlite3.connect(self.db_path) as conn:
                cursor = conn.cursor()
                cursor.execute('''
                    DELETE FROM ingredient_lists 
                    WHERE id = ? AND user_id = ?
                ''', (list_id, user_id))
                conn.commit()
                return cursor.rowcount > 0
        except Exception as e:
            print(f"Error deleting ingredient list: {e}")
            return False
